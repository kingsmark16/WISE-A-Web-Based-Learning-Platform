...
const Student = () => {
    ...
    if (isLoading) {
        ...
        <div className="h-screen w-full flex items-center justify-center">
            ...
            <p className="mt-4">Loading student dashboard...</p>
            </div>
        </div>
        )
    }
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Studnet Dashboard</h1>
              {data && (
                <p className="mt-2 text-lg text-green-600 font-medium">{data.message}</p>
              )}
            </div>
      ...
      <main className="max-w-7xl mx-auto py-6 px-4">
        <div className="bg-white rounded-lg shadow p-6">
          <p>{data.message}</p>
          <p>{data.response.emailAddress}</p>
        </div>
      </main>
    </div>
  )
